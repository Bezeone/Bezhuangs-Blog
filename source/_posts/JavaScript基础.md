...
- JavaScript 是一门跨平台、面向对象的脚本语言，不需要编译成字节码，由浏览器直接解析并执行。
- JavaScript 是用来控制网页行为的，它能使网页可交互，如改变页面内容、修改指定元素的属性值、对表单进行校验等
- JavaScript 和 Java 是完全不同的语言，不论是概念还是设计，只是名字比较像而已。但是基础语法类似，所以有 Java 的学习经验，再学习 JavaScript 语言就相对比较容易些
...
- ECMAScript 6 (简称ES6) 是最新的 JavaScript 版本（发布于 2015 年)
...
- 外部脚本不能包含 `<script>` 标签，在 js 文件中直接写 js 代码即可，不要在 js文件 中写 `script` 标签
...
- 原始数据类型：
  ...
  - NaN 是一个特殊的 number 类型的值（Not a number）
  - string：字符、字符串，单双引皆可
...
- JS中，函数调用可以传递任意个数参数，例如  `let result = add(1,2,3);` ，它是将数据 1 传递给了变量a，将数据 2 传递给了变量 b，而数据 3 没有变量接收。
...
2. 从弹框输出的内容，也可以看出是一个图片元素对象。

3. 根据标签名称获取所有的 `div` 元素对象
...
4. 获取所有的满足 `name = 'hobby'` 条件的元素对象

   ```js
   //4. getElementsByName：根据name属性值获取，返回Element对象数组
...
5. 获取所有的满足 `class='cls'` 条件的元素对象

   ```js
   //5. getElementsByClassName：根据class属性值获取，返回Element对象数组
