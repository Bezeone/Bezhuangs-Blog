...
> 操作系统（02327）实践环节是操作系统专业课的上级测试部分，考核目标是具有熟练使用 Linux 系统常用命令的能力以及初步掌握进程的创建、控制及进程间通信程序的编写方法。运行环境是通过 putty 远程登录工具远程连接 Linux 服务器，在练习时我仍旧使用系统默认终端（zsh）连接[云服务器](/Ubuntu远程开发配置)使用。
