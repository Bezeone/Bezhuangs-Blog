...
> 堆栈（stack）又称为栈或堆叠，是只允许在一端进行插入或删除操作的线性表，遵循先进后出 FILO（First In Last Out）的原则。队列（Queue）简称队，也是一种操作受限的线性表，只允许在表的一端进行插入，而在表的另一端进行删除。向队列中插入元素称为入队或进队，删除元素称为出队或离队。
